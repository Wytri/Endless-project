...
@export var is_special_scene: bool = false  # Indica si es la escena especial
...
var allow_jump = true # Nueva variable para controlar si se permite el salto
...
func _move(delta):
	...
	# Solo permitir salto si `allow_jump` es true
	if allow_jump and Input.is_action_just_pressed("saltar"):
		if character.is_on_floor():
			_current_movement = _movements.JUMP_WITH_SWORD
			_is_jumping = true
			_jump_count += 1 # Primer salto
		elif _is_jumping and _jump_count < _max_jumps:
			_current_movement = _movements.JUMP_WITH_SWORD
			_jump_count += 1 # Segundo salto
...
# Llamar esta función en la escena especial para desactivar el salto
func disable_jump():
	allow_jump = false

# Llamar esta función en las otras escenas para activar el salto
func enable_jump():
	allow_jump = true
